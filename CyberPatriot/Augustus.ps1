...
if ($option -eq 2) {
    ...
    Catch {"Default accounts were not renamed (cuz they already were dumbass)"}
	...
	if (Test-Path $path1) {
		...
		if ($ausers -match $user1 -or $user1 -eq $user) {
            ...
            net user $user1
            if ((net user $user1 | findstr /c:"Account expires") -eq "Account expires              Never") {"disabled PASSWORDNEVEREXPIRES for $user1"}
...
if ($option -eq 6) {
    ...
    "Flushing Hosts..."
    if (!(Test-Path C:\Users\$user\Desktop\AugustusOut\hosts.txt)) {New-Item -Path C:\Users\$user\Desktop\AugustusOut\hosts.txt}
